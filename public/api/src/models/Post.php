...
//ok
	...
	public function showSearchPosts($search) {
		$query = $this->db->prepare("SELECT DISTINCT Post.* FROM Post JOIN Tagge ON Post.id = Tagge.idPost JOIN Tag ON Tagge.idTag = Tag.id JOIN Sujet ON Post.sujet = Sujet.id WHERE Post.titre LIKE '%".$search."%' OR Post.texte LIKE '%".$search."%' OR Post.auteur LIKE '%".$search."%' OR Sujet.titre LIKE '%".$search."%' OR Tag.nom LIKE '%".$search."%';");
		$query->execute();
		$result = $query->fetchAll();
		return $result;
	}
